  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">{t("inviteCode")}: <span className="font-mono font-bold text-primary">SHAKTI-2024</span></p>
      </div>

      <div className="grid grid-cols-3 gap-4">
        {[
          { icon: Users, label: t("members"), value: "5" },
          { icon: IndianRupee, label: t("groupCorpus"), value: "₹24,500" },
          { icon: Calendar, label: t("nextMeeting"), value: "8 Mar" },
        ].map((s, i) => (
          <motion.div key={i} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} className="bg-card rounded-2xl p-4 border border-border text-center">
      ...
      <div>
        <h3 className="font-bold text-sm mb-3">{t("members")}</h3>
          ...
          {membersList.map((m, i) => (
            <motion.div key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }} className="bg-card rounded-xl p-3 border border-border flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-full bg-primary/10 flex items-center justify-center text-sm font-bold text-primary">{m.name[0]}</div>
                <div>
      ...
      <div className="bg-muted rounded-2xl p-4">
        <h3 className="font-bold text-sm mb-2 flex items-center gap-2">
          <MessageCircle className="w-4 h-4" /> {t("recentAnnouncements")}
        </h3>
