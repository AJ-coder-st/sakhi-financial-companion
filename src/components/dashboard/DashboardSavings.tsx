    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        {goals.map((g, i) => {
          ...
          return (
            <motion.div key={i} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.1 }} className="bg-card rounded-2xl p-5 border border-border hover:shadow-sakhi transition-shadow">
              ...
              <div className="relative w-full h-24 bg-muted rounded-xl overflow-hidden mb-3">
                <motion.div initial={{ height: 0 }} animate={{ height: `${pct}%` }} transition={{ delay: 0.3 + i * 0.1, duration: 0.8 }} className="absolute bottom-0 left-0 right-0 bg-primary/20 rounded-b-xl" style={{ background: `linear-gradient(180deg, hsl(var(--primary) / 0.15), hsl(var(--primary) / 0.35))` }} />
              ...
              <Button size="sm" variant="outline" className="w-full mt-3">{t("addSavingsBtn")}</Button>
            </motion.div>
          );
        })}
      </div>
